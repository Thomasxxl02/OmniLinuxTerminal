...
export const TerminalView: React.FC<TerminalViewProps> = ({
  ...
  const controller = useTerminalController(onUpdateTab);
  const { isExecuting, riskConfirm } = controller;
  ...
  // ==== Exécution de commande (déléguée au hook useTerminalController, source unique) ====
  const executeCommand = (command: string, opts: { skipRisk?: boolean } = {}) =>
    controller.runCommand(command, tab, opts);
  ...
  const handleRiskDecision = (run: boolean) => {
    if (!controller.riskConfirm) return;
    const command = controller.riskConfirm.command;
    if (!run) {
      controller.dismissRisk();
      return;
    }
    setInputVal('');
    setHistoryIdx(-1);
    void controller.runConfirmed(command, tab);
  };
  ...
  return (
    ...
    >
      ...
      {riskConfirm && (
        <RiskConfirmModal
          command={riskConfirm.command}
          report={riskConfirm.report}
          onRun={() => handleRiskDecision(true)}
          onCancel={() => handleRiskDecision(false)}
        />
      )}
    </div>
  );
